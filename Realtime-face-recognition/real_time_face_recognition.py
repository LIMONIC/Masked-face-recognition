        ...
        if ret is True:
            ...
            if len(bboxes) > 0:
                for num, bbox in enumerate(bboxes):
                    ...
                    # cv2.rectangle(img, (bbox[0] - bbox[2] // 2, bbox[1] - bbox[3] // 2), (bbox[0] + bbox[2] // 2, bbox[1] + bbox[3] // 2), color, 2)
                    cv2.rectangle(img, (bbox[0], bbox[1]), (bbox[0] + bbox[2], bbox[1] + bbox[3]), color, 2)
                    # cv2.putText(img, "%s: %.2f" % (id2class[class_id], re_confidence[num]), ((bbox[0] - bbox[2] // 2) + 2, (bbox[1] - bbox[3] // 2) - 2),
                    #             cv2.FONT_HERSHEY_SIMPLEX, 0.8, color)

                    # ----face recognition
                    name = ""
                    if len_ref_path > 0:
                        # img_fr = img_rgb[bbox[1] - bbox[3] // 2:bbox[1] + bbox[3] // 2, bbox[0] - bbox[2] // 2:bbox[0] + bbox[2] // 2, :]  # crop
                    ...
                    # cv2.putText(img, "{},{}".format(id2class[class_id], name), ((bbox[0] - bbox[2] // 2) + 2, (bbox[1] - bbox[3] // 2) - 2),
                    #             cv2.FONT_HERSHEY_SIMPLEX, 0.8, color)
            ...
            #----image display
            cv2.imshow("demo of masked face recognition", img)
            ...
            if key == ord('q'):
                ...
                if len(bboxes) > 0:
                    # img_temp = img[bbox[1] - bbox[3] // 2:bbox[1] + bbox[3] // 2 , bbox[0] - bbox[2] // 2:bbox[0] + bbox[2] // 2, :]
...
if __name__ == "__main__":
    ...
    # pb_path = r"C:\Users\ztyam\OneDrive - University of Florida\0_UF Class\2021 Spring\03_Pattern Recognition\Project\TrainedModels\FaceNet_masked_pic_aug2_new\pb_model.pb"
    ...
    ref_dir = r"C:\Users\ztyam\OneDrive - University of Florida\0_UF Class\2021 Spring\03_Pattern Recognition\Project\dataSet\tes_database_4_small"
